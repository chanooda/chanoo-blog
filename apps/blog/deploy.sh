...
IMAGE_NAME="blog-service-app"
IMAGE_TAG="latest"
FULL_IMAGE_PATH="$REGISTRY_URL/$IMAGE_NAME:$IMAGE_TAG"

APP_PORT=3000
APP_NAME=blog-service
TARGET_INC="/etc/nginx/conf.d/blog_service_target.inc"
NGINX_CONF="/etc/nginx/conf.d/blog_service.conf"
...
do
 ...
 STATUS_CODE=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 10 http://127.0.0.1:$TARGET_PORT || echo "000")
