...
import type { Series, Tag, Write, WriteFilterForm } from "utils";
...
export function PostList({ writes, series, tags }: PostListProps) {
	...
	const setQueries = (chooseSeriesId = "", chooseTagId = "") => {
		router.push(
			`/post?search=${search}&seriesId=${chooseSeriesId}&tagId=${chooseTagId}`,
		);
	};
			...
			<Stack>
				<Stack maxWidth={800} mx="auto" position="relative" width="100%">
					...
					{writes?.length > 0 ? (
						...
						>
							{writes?.map((write) => (
								<Grid2 key={write.id} size={{ xs: 8, sm: 8, md: 8 }}>
