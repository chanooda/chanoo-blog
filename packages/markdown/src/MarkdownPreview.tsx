...
export function MarkdownPreview({ children, handleGetId, write }: MdPreviewProps) {
  return (
    <Box fontFamily="inherit" height="100%" maxWidth={800} mx="auto" py={4} width="100%">
      <Stack direction="column" width="100%">
        ...
        {write?.createdAt && (
          <Stack direction="row" gap={1} mt={2}>
            <Typography fontWeight={600}>김찬우</Typography> <Typography> • </Typography>
        ...
        {write.tag.length > 0 && (
          <Stack direction="row" flexWrap="wrap" gap={2} mt={2} width="100%">
            {write?.tag?.map((tag) => {
              if (typeof tag === 'string') return <Chip color="primary" key={tag} label={tag} />;
              return (
                <Chip
                  color="primary"
                  key={tag.tag.id}
                  label={tag.tag.name}
                  onClick={() => handleGetId && handleGetId('tag', tag.tag.id)}
                />
              );
            })}
          </Stack>
          ...
          >
            {typeof write.series === 'string' ? (
              ...
              <Typography
                ...
                variant="h6"
                onClick={() =>
                  handleGetId && handleGetId('series', (write.series as SeriesDetail).id)
                }
              >
                {write.series.name}
              </Typography>
                  ...
                  return (
                    <Typography
                      ...
                      sx={{
                        ...
                        cursor: 'pointer',
                        ...(write?.id === orderedWrite.id && { fontWeight: 700 })
                      }}
                      onClick={() => handleGetId && handleGetId('post', orderedWrite.id)}
                    >
